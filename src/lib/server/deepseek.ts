...
// PER-PROCESS CONCURRENCY CAP ON OUTBOUND DEEPSEEK CALLS. WITHOUT THIS, PARALLEL CHAPTER JOBS (THE CURRENT
// READ + BACKGROUND PREFETCH WARM-UPS) FAN OUT UNTHROTTLED AND CAN TRIP RATE LIMITS / SPIKE COST.
// NOTE (Phase 5): WHEN REDIS/BullMQ IS CONFIGURED, THE *AUTHORITATIVE GLOBAL* CAP IS THE BullMQ WORKER
// concurrency (runtime-adjustable via redis.setGlobalConcurrency + a DEEPSEEK_RPM limiter); THIS PQueue
// THEN ACTS ONLY AS A PER-WORKER-PROCESS SAFETY THROTTLE, SO SET DEEPSEEK_CONCURRENCY >= THE WORKER cap.
// IN THE SINGLE-INSTANCE (NO-REDIS) BRIDGE, THIS PQueue IS THE GLOBAL CAP.
