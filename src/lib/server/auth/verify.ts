...
// THE FIREBASE SESSION-COOKIE NAME (WEB AUTH). NATIVE USES AN Authorization: Bearer HEADER INSTEAD.
...
// VERIFY A FIREBASE ID TOKEN (BEARER AUTH — NATIVE, OR THE SHORT-LIVED TOKEN POSTED AT SIGN-IN).
export function verifyIdToken(idToken: string): Promise<DecodedIdToken> {
	return adminAuth().verifyIdToken(idToken);
}

// MINT A LONG-LIVED SESSION COOKIE FROM A FRESH ID TOKEN (WEB SIGN-IN). THE ID TOKEN MUST BE < 5 MIN OLD.
...
// VERIFY A SESSION COOKIE (WEB AUTH). checkRevoked DEFAULTS false SO THE PER-REQUEST hook VERIFIES LOCALLY
