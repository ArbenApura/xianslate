<script lang="ts">
	...
	// IDLE (JUST DISABLED), SO THE TWO SIGN-IN PATHS ARE VISUALLY DISTINCT.
	let busyEmail = false;
		...
		<div class="w-full max-w-sm">
			...
			>
				...
				<div class="mb-6 text-center">
					<h1 class="font-[Literata,Georgia,serif] text-2xl font-bold tracking-tight">Welcome back</h1>
					<p class="mt-1.5 text-sm opacity-60">Sign in to your Xianslate library.</p>
				</div>

				<!-- EMAIL / PASSWORD — outline-none + focus:ring IS THE SANCTIONED KEYBOARD-FOCUS PATTERN -->
				<form class="flex flex-col gap-4" on:submit|preventDefault={signInEmail}>
					...
					<div class="flex flex-col gap-1.5">
						<div class="flex items-center justify-between text-xs font-medium opacity-70">
							<span>Password</span>
							<button
								type="button"
								use:ripple
								...
								disabled={busy}
								class="rounded font-medium text-[#b23a2e] hover:underline disabled:opacity-50 dark:text-[#e08a63]"
					...
					>
						{#if busyEmail}Signing in…{:else}<LogIn size={16} /> Sign in{/if}
					</Button>
				</form>
				...
				<!-- GOOGLE — REAL MULTI-COLOUR "G" MARK (INLINE SVG, BRAND COLOURS ARE FIXED, NOT THEME-DEPENDENT);
				     SWAPS TO A SPINNER + "Signing in…" WHILE AUTH IS IN FLIGHT -->
				<button
				...
				<p class="mt-6 text-center text-sm opacity-60">
					...
					<a
						href="/signup/"
						use:ripple
						class="rounded font-medium text-[#b23a2e] hover:underline dark:text-[#e08a63]"
						>Create an account</a
					>
				</p>
			</div>
