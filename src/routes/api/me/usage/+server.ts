...
// SERVER-AUTHORITATIVE DATA (getAccountUsage); requireUser KEEPS IT SELF-ONLY, AND THE hooks 401 GUARD
// SHIELDS IT FOR SIGNED-OUT CALLERS.
export const GET: RequestHandler = async ({ locals }) => {
	const user = requireUser(locals);
	return json({ usage: await getAccountUsage(user.id) });
};
