<script lang="ts">
	...
	async function load() {
		...
		try {
			...
			errMsg = 'Couldn’t load the dashboard. Try again.';
		} finally {
			loading = false;
		}
	}
...
<svelte:head><title>Sites & AI cost · Admin · Xianslate</title></svelte:head>
...
<div class="mx-auto max-w-5xl px-4 py-6">
	...
	{#if loading}
		...
		<div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
			{#each [0, 1, 2, 3] as i (i)}
				<div class="h-20 animate-pulse rounded-xl bg-black/10 dark:bg-white/10"></div>
			{/each}
		</div>
	{:else if errMsg}
		<!-- ERROR STATE -->
	...
	{:else if dash}
		...
		<section class="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
			<!-- TOTAL AI COST -->
			<div class={CARD}>
				<div class={cn('mb-1 flex items-center gap-1.5 text-[12px]', MUTED)}>
					<Coins size={14} /> Total AI cost
				</div>
