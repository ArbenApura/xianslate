...
line(`rows=${agg.n}  cost=$${Number(agg.cost).toFixed(4)}  avg/row=$${(Number(agg.cost)/Math.max(1,agg.n)).toFixed(5)}`);
const promptT = Number(agg.prompt), cachedT = Number(agg.cached), compT = Number(agg.completion);
...
const PIN=0.27/1e6, PHIT=0.027/1e6, POUT=1.10/1e6;
const miss = promptT - cachedT;
const cIn = miss*PIN, cHit = cachedT*PHIT, cOut = compT*POUT;
...
const ch = (await q(`SELECT COUNT(*) n,
   SUM(content_target IS NOT NULL) translated,
   SUM(extracted_at IS NOT NULL) extracted FROM chapters`))[0];
line('\n=== chapters ===');
...
const sz = (await q(`SELECT AVG(LENGTH(content_source)) avgsrc, AVG(LENGTH(content_target)) avgtgt
   FROM chapters WHERE content_target IS NOT NULL`))[0];
...
const dist = (await q(`SELECT
   ...
   AVG(prompt_tokens) ap, AVG(cached_tokens) ac FROM translations WHERE completion_tokens>0`))[0];
line('\n=== per-row token distribution (body) ===');
line(`completion: min=${dist.mn} max=${dist.mx} avg=${Math.round(dist.av)}`);
line(`prompt avg=${Math.round(dist.ap)}  cached avg=${Math.round(dist.ac)}  cache-hit/row=${(dist.ac/Math.max(1,dist.ap)*100).toFixed(0)}%`);
