...
for (const name of Object.keys(all).filter((n) => /\.x?html?$/i.test(n))) {
	...
	const hCounts = {};
	for (const m of body.matchAll(/<(h[1-6])\b/gi)) hCounts[m[1].toLowerCase()] = (hCounts[m[1].toLowerCase()] ?? 0) + 1;
	...
	console.log(`${name}  text=${textLen}  h=${JSON.stringify(hCounts)} section=${sections} chapWordOccur=${chapterWords} pbHints=${pageBreaks}`);
	// SHOW FIRST FEW HEADINGS IF ANY
	const heads = [...body.matchAll(/<(h[1-6])\b[^>]*>([\s\S]*?)<\/\1>/gi)].slice(0, 8);
	for (const h of heads) console.log(`      <${h[1]}> ${h[2].replace(/<[^>]+>/g, '').trim().slice(0, 60)}`);
}
