...
```svelte
<!-- TIMELINE ITEMS --> ✓ <!-- Timeline items --> ✗ // FEATURE CARD DATA ✓ // Feature card data ✗
```
...
-   **Default to `dark:` variants.** `dark:` only splits the dark group vs the light group — it _cannot_ tell `sepia` from `light`, nor `oled`/`contrast` from `dark`. Use it for the common two-state case.
...
```svelte
// IMPORTED MODULES import {(settings, isDarkTheme)} from '$lib/stores/settings'; import {cn} from '$lib/utils/cn';
...
<div
	style="background: linear-gradient(to bottom, transparent, {isDarkTheme($settings.theme) ? '#0e131c' : '#f4ecd8'});"
></div>
```
...
```svelte
// IMPORTED MODULES import {ripple} from '$lib/actions/ripple';
...
const handleSubmit = () =>
	toast.promise(saveProfile(data), { loading: 'Saving...', success: 'Saved!', error: 'Failed' });
```
