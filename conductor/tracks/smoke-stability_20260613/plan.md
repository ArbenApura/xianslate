...
-   [ ] Task 2.1: ⌨ `npm run check` → `0 errors`.
-   [ ] Task 2.2: ⌨ `npm run lint` → exit 0 (the single pre-existing `headless.ts` `no-var` *warning* is OK).
-   [ ] Task 2.3: ⌨ `npm run db:generate` → prints "No schema changes" / generates nothing new (i.e. the
        committed migrations match `schema.ts`; `git status` shows no new `drizzle/` files).
-   [ ] Task 2.4: ⌨ `npm run build` (adapter-node) → `✓ built`, "Using @sveltejs/adapter-node".
-   [ ] Task 2.5: ⌨ `BUILD_TARGET=capacitor npm run build` → `✓ built`, "Wrote site to build-capacitor".

### Verification

-   [ ] All five green; no schema drift. **Findings:** _(…)_
