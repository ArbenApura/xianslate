# Conductor — Xianslate

Navigation hub for project context.

## Quick Links

-   [Product Definition](./product.md)
-   [Product Guidelines](./product-guidelines.md)
-   [Tech Stack](./tech-stack.md)
-   [Workflow](./workflow.md)
-   [Tracks](./tracks.md)
-   [Code Style Guides](./code_styleguides/) — [Svelte](./code_styleguides/svelte.md) · [TypeScript](./code_styleguides/typescript.md)

## Active Tracks

-   [x] [xianslate-mvp_20260611](./tracks/xianslate-mvp_20260611/index.md) — Xianslate MVP: Fetch, Glossary, Translate & Reader (6/6 phases ✅)
-   [~] [scale-up-multitenant_20260613](./tracks/scale-up-multitenant_20260613/index.md) — Multi-User Scale-Up & Cross-Platform Delivery (1/8 phases · Phase 0 done)

<!-- Auto-populated by /conductor:new-track -->

## Getting Started

Run `/conductor:new-track` to create your first feature track.

A detailed implementation plan for the initial build also lives at
`C:\Users\Admin\.claude\plans\kind-marinating-bird.md`.
